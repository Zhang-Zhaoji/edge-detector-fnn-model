...
class FCNN(nn.Module):
    def __init__(self, in_channels=25, hidden_channels_1=32,hidden_channels_2=16, out_channels=3) -> None:
        super().__init__()
...
class Edge_Dataset(Dataset):
    def __init__(self,X_path,Y,transform = None) -> None:
        ...
        super().__init__()
        self.X_path = X_path
        Y_dataset = np.zeros((len(Y),3))
        array_Y = np.array(Y)
        Y_dataset[array_Y==1,0] = 1
        Y_dataset[array_Y==0,1] = 1
        Y_dataset[array_Y==2,2] = 1
        if np.sum(Y_dataset) == 0:
            raise RuntimeError("what happened? there are no any data")
        self.Y = Y_dataset
...
def train(model = FCNN(in_channels=25,hidden_channels_1=32,hidden_channels_2=16,out_channels=3)):
    '''
    ...
    print("Loading data!")
    code_path = os.getcwd()
    train_path = os.path.dirname(code_path) + "\\traindata_point\\"
    ...
    with open(train_path+"on_corner.txt","r") as file2:
        on_corner_paths = file2.readlines()
    on_corner_paths = list(map(lambda x: data_path + x, on_corner_paths))
    on_corner_length = len(on_corner_paths)

    labels = [1] * on_length + [0] * off_length + [2] * on_corner_length
    labels = np.array(labels)
    data_paths += off_data_paths
    data_paths += on_corner_paths
    data_paths = list(map(lambda x: x.replace("\n",""),data_paths))
    print(on_length,off_length,on_corner_length)
    ...
    print(len(train_data_label),len(valid_data_label))
    ...
    edge_dataloader = DataLoader(dataset=edgedataset,batch_size=batch_size,shuffle=True, num_workers=16)
    edge_dataloader
    valid_dataloader = DataLoader(dataset=valid_dataset,batch_size=batch_size,shuffle=True, num_workers=16)
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Accuracy of the network on the {len(train_data_path)} train images: {100 * train_correct / train_total:.5f} %')
        ...
        test_acc.append(100 * correct / total)
            # forward
        PATH = f'./{epoch+1}_point_edge_detector.pth'
        torch.save({
            'epoch': epoch,
    ...
    with open('point_example.txt', 'w') as file:
...
def predict(graph_path , model_path, model = FCNN(in_channels=25,hidden_channels_1=32,hidden_channels_2=16,out_channels=3)):
    #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    checkpoint = torch.load(model_path)
    model.load_state_dict(checkpoint["model_state_dict"])
    #model = model.to(device)
    ...
    for index in trange(len(graph_list)):
        ...
        for k in range(2,2+x):
            for j in range(2,2+y):
                ...
                if predicted_vector[:,2].item() > 0:
                    corners.append((j,k))
        corners = np.array(corners)
        #print(corners)
        print(corners.shape)
        plt.imshow(output[0,:,:])
        plt.scatter(corners[:,0],corners[:,1],c="r",marker="*")
        #plt.show()
        plt.savefig(f"graph3/{name}.jpg")
        plt.close("all")
...
if __name__ == '__main__':
    ...
    if args.run == 'train':
        ...
        if args.graph_path != 'None' and  args.model_path != 'None':
            ...
            predict("../test_images/natural_scene", "./50_point_edge_detector.pth")
